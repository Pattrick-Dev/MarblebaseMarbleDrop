...
/** Hologram + particles for Upgrade Stations, matching InfusionTableAmbient/RecyclerAmbient. */
public final class UpgradeStationAmbient {
    ...
    // Track hologram stands by station key
    private final Map<String, UUID> markerIds = new HashMap<>();

    public UpgradeStationAmbient(Plugin plugin, UpgradeStationManager stations) {
        this.plugin = plugin;
    ...
    public void removeStation(Location stationLoc) {
        if (stationLoc == null) return;

        String key = stations.keyOf(stationLoc);
        ...
        Location standLoc = stationLoc.clone().add(0.5, getHologramYOffset(), 0.5);
        if (isChunkLoaded(standLoc)) {
            removeDuplicateHolograms(standLoc.getWorld(), standLoc, getHologramName(), null);
        }
    }
    ...
    private void tick() {
        ...
        for (Location loc : stations.getStationLocations()) {
            World w = loc.getWorld();
            if (w == null) continue;

            String key = stations.keyOf(loc);
            liveKeys.add(key);
            ...
            int endRod = active ? 4 : 1;
            int spark = active ? 3 : 1;

            w.spawnParticle(Particle.END_ROD, base, endRod, 0.20, 0.12, 0.20, 0.01);
            w.spawnParticle(Particle.ELECTRIC_SPARK, base, spark, 0.20, 0.12, 0.20, 0.02);

            if (active && (Bukkit.getCurrentTick() % 140 == 0)) {
                for (Player p : nearby) {
                    p.playSound(p.getLocation(), Sound.BLOCK_SMITHING_TABLE_USE, 0.2f, 1.1f);
                }
            }
        }

        // Remove markers for stations that no longer exist (only if we can see the entity)
    ...
    private String getHologramName() {
        if (plugin instanceof Main main) {
            String s = main.cfg().upgradeHologramName();
            if (s != null && !s.isBlank()) return s;
        }
        return ChatColor.AQUA + "✦ " + ChatColor.LIGHT_PURPLE + "Upgrade Station" + ChatColor.AQUA + " ✦";
    }

    private double getHologramYOffset() {
        if (plugin instanceof Main main) {
            return main.cfg().upgradeHologramYOffset();
        }
        return 1.2;
    }
